...
function FormRegis() {
  ...
  function handleFormSubmit(e) {
    ...
    if (!namaDepan || !namaBelakang || !email || !password || !confirmPass) {
      ...
      !validPassword.test(confirmPass)
    ) {
      toast.error(
        "Password minimal harus terdiri dari 8 karakter dengan 1 huruf kapital, simbol dan angka"
      );
      ...
      api
        ...
        .then((res) => {
          toast.success(
            "Pendaftaran berhasil! Silahkan masuk dengan akun yang telah didaftarkan"
          );
      ...
      <Row>
        ...
        <Col lg={5} className="d-flex justify-content-center flex-column">
          ...
          <Form>
            ...
            <Form.Select
              value={bidang}
              onChange={(event) => setBidang(event.target.value)}
              style={{ width: "100%", borderRadius: 30, textAlign: "center" }}
            >
              <option>Pilih Bidangmu</option>
              {kelas === ""
                ? null
                : kelas.map(({ _id, bidang_seni }, index) => (
            ...
            <Form.Check
              type="checkbox"
              label="Saya setuju dengan ketentuan dan syarat"
            />
          </Form>
          <br />
          <Button
